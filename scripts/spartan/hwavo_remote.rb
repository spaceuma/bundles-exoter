...
options = {:nav => true,
           ...
           :loc => true,
           :v => false,
           :spartan => true,    # only effective if v=false
           :spartan_dem => true,
           :autonav => true}
...
# Execute the task
Orocos::Process.run 'control', 'navcam', 'loccam', 'imu', 'tmtchandling', 'unit_vicon', 'navigation', 'unit_visual_odometry', 'fdir', 'autonomy', 'spartan::Task' => 'spartan', 'spartan::MappingTask' => 'spartan_dem', 'vicon::Task' => 'vicon_logger' do
    ...
    vicon_logger = TaskContext.get 'vicon_logger'
    Orocos.conf.apply(vicon_logger, ['default', 'exoter'], :override => true)
    vicon_logger.configure
    ...
    if options[:v] == true
        ...
        if options[:spartan] == true
            visual_odometry = TaskContext.get 'spartan'
            #visual_odometry = Orocos.name_service.get 'spartan'
            visual_odometry.apply_conf_file("/home/marta/rock/perception/orogen/spartan/config/spartan::Task.yml", ["default"])
        else
        ...
        viso2_with_imu = TaskContext.get 'viso2_with_imu'   # Equivalent for SPARTAN?
    ...
    if options[:autonav]
        ...
        autonav.configure
        if options[:spartan_dem] == true
            spartan_dem = TaskContext.get 'spartan_dem'
            spartan_dem.apply_conf_file("/home/marta/rock/perception/orogen/spartan/config/spartan::MappingTask.yml", ["default"])
            spartan_dem.configure
        end
    end
    ...
    if options[:nav] == true
        ...
        if options[:autonav]
            ...
            if options[:spartan_dem] == true
                spartan_dem.img_in_left.connect_to          autonav.mapper_left_frame_out
                spartan_dem.img_in_right.connect_to         autonav.mapper_right_frame_out
                spartan_dem.world2body.connect_to           autonav.mapper_navcam2world_out
                spartan_dem.map_out.connect_to              autonav.pointcloud_in
            end
    ...
    if options[:v] == true
        ...
        if options[:spartan] == true
            camera_loccam.left_frame.connect_to                 visual_odometry.img_in_left
            camera_loccam.right_frame.connect_to                visual_odometry.img_in_right
            #command_arbiter.motion_command.connect_to           visual_odometry.motion_command

            visual_odometry.delta_vo_out.connect_to             viso2_with_imu.delta_pose_samples_in
            ...
            vicon_logger.pose_samples.connect_to                viso2_evaluation.groundtruth_pose
            vicon_logger.pose_samples.connect_to                viso2_evaluation.groundtruth_pose_not_aligned
            if options[:autonav]
    ...
    if options[:autonav]
        autonav.start
        if options[:spartan_dem]
            spartan_dem.start
        end
    end

    vicon_logger.log_all_ports
